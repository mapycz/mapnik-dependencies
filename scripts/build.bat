...
:LIBXSLT
CALL scripts\build_libxslt.bat
IF %ERRORLEVEL% NEQ 0 GOTO ERROR
...
:: Skip NODE
goto DONE
IF %FASTBUILD% EQU 1 ECHO FASTBUILD skipping NODE && GOTO NODEMAPNIK
...
:: Skip NODEMAPNIK
goto DONE
IF %FASTBUILD% EQU 1 ECHO FASTBUILD skipping NODEMAPNIK && GOTO LIBOSMIUM
SET PUBNODE=
...
:: Skip LIBOSMIUM
goto DONE
IF %FASTBUILD% EQU 1 ECHO FASTBUILD skipping LIBOSMIUM && GOTO DONE
